...
class CarController {
    ...
    CollectionModel<EntityModel<Car>> list() {
        var models = service.list().stream().map(assembler::toModel)
     ...
     */
    @PostMapping
    ResponseEntity<?> post(@Valid @RequestBody Car car) throws CarNotFoundException, NoSuchElementException {
        var saved = service.save(car);
